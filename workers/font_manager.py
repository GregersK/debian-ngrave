...
def get_strokes(tekst, tekst_hoejde_mm, font='block'):
    if font == 'block':
        return block_strokes(tekst, tekst_hoejde_mm)
    return hershey_strokes(tekst, tekst_hoejde_mm, font)

def hershey_strokes(tekst, tekst_hoejde_mm, font_name):
    try:
    ...
    for ch in tekst:
        ...
        try:
            ...
            x_cursor += glyph.get('width', 16) * scale
        except:
            x_cursor += 8 * scale
